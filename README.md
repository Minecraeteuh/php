...
4. Créer une base de données nommée `utilisateurs`
5. Importer le fichier `database.sql` fourni dans cette base de données (onglet Importer)
6. Ouvrir `configphp.php` et vérifier les paramètres de connexion (voir ci-dessous)
7. Accéder au site : [http://localhost/nom_du_dossier/index.php](http://localhost/nom_du_dossier/index.php)
...
| realisateur_id | INT FK        | Référence vers `realisateurs`   |
...
|----------|-----------|------------------------------|
| id       | INT PK AI | Identifiant unique           |
| movie_id | INT FK    | Référence vers `films`       |
| genre_id | INT FK    | Référence vers `genres`      |
...
|-----------|--------|--------------------------|
| movie_id  | INT FK | Référence vers `films`   |
| acteur_id | INT FK | Référence vers `acteurs` |
